...
> Transformer Decoder를 PyTorch로 직접 구현하고,
...
저작권 보호기간이 끝난 한국 작가의 소설을 한국어 위키문헌 API에서 수집하는 파이프라인을
구현했다. 모델 구조와 문자 단위 토큰화를 유지해 한국어 문체 학습에 집중했다.
...
- 출처와 revision이 기록된 재현 가능한 한국어 코퍼스를 구축했다.
